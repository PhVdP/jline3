...
public class BindingReader {
	...
	//PV Updated with reader
	public BindingReader(Reader reader) {
	...
	public int readCharacter() {
		...
		try {
			...
			while (c == NonBlockingReader.READ_EXPIRED) {
				// PV check if the reader is ready if not wait...
				if (reader instanceof NonBlockingReader) {
					c = ((NonBlockingReader)reader).read(100L);
				} else {
					if (reader.ready ()) {
						c = reader.read ();
					} else {
	...
	public int peekCharacter(long timeout) {
		...
		try {
			//PV change with the reader
			if (reader instanceof NonBlockingReader) {
				return ((NonBlockingReader)reader).peek(timeout);
			} else {
				if (reader.ready ()) {
					c = reader.read ();
